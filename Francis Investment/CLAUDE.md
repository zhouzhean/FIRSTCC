...
**v2.2**（2026-05-25）：全自动调度器 + UI 重构。Simfolio 置顶为实时交易看板（倒计时+交易动态推送），6个报告板块合并为"交易分析与报告"，持仓分析暂时灰掉，工具栏"运行分析"替换为 AI 交易员状态指示灯。桌面快捷方式静默启动（VBS 无终端弹窗），日历去装饰化，工具栏去冗余倒计时。
...
- 功能：双击 → 静默打开 Chrome 独立窗口到 `http://127.0.0.1:8765`，无终端弹窗
...
- 功能：开机后台静默启动 Mosaic Server，无终端窗口

### 架构说明
- `launch.vbs` — 静默执行 `start.bat`（启动服务器 + 检测端口），window mode 0（隐藏）
- `open.vbs` — 静默启动 Chrome `--app` 模式，window mode 1（显示浏览器）
- 两个 VBS 分开：桌面快捷方式只开浏览器（快），开机自启只开服务器（后台）
- 之前用 `wscript.exe` + 参数传 .vbs 路径会因 Windows Shell 引号处理出错导致无法启动，改为直接指向 .vbs 文件即可
