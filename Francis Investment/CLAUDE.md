...
- **P0-2 — Repaired Trade Simulator**: 3 equal-weight sleeves, overlapping daily cohorts, T+1 entry→T+4 exit, NAV extended through last exit. 34 fixture tests (all pass)
...
### P0.2 Research Output Interpretability — CONDITIONAL PASS (4/5 done, T5 pending)

- **T1 — Benchmark strict availability** ✅: `benchmarkStatus=available` only when `benchmarkTradeCount>0` && `benchmarkUnavailableCount!=null`. Otherwise `unavailable`, `netExcessReturn=null`, UI shows "指数基准暂不可比较"
- **T1-fix — Residual null semantics** ✅ (2026-06-24): `normalizeBenchmarkFields()` — when `benchmarkStatus !== "available"`, ALL benchmark return + excess fields are strictly `null` (never `|| 0`). `avgNetExcess` only from available windows. Cockpit shows `Excess=N.A.` in gray when unavailable. Regression test: 17/17 pass.
- **T2 — Coverage dual constraint** ✅: `expectedCalendarDates=649`, `actualExchangeOpenDays=636`, `holidayExclusions=13` (with dates), `trueDataGap=0`. Constraint 1: 649=636+13 ✅. Constraint 2: 636=636+0 ✅
- **T3 — Cohort API consistency** ✅: `prediction-settlement` and `cohort-integrity` both filter `legacyNoTargetDate`, counts identical (legacyNoTargetDate=459, canonical=0, intraday=0, globalBlocked=0)
- **T4 — Release identity** ✅: Commit `43c4f8e`, deploy_manifest.json updated (13 files), cloud `identityStatus=matched`, `gitCommit=deployCommit=43c4f8e`
- **T5 — 09:30 runtime verification** ⏳: Tomorrow 2026-06-25, scheduler naturally triggers canonical full scan. Verify `canonicalCohortCount=50`, all 50 entries have all 7 required fields non-null → then change to PASS
...
- **Same-path benchmark sleeve**: parallel benchmark sleeves mirror strategy (same dates/structure/compounding), tracks SH index return. Output: `netReturn`, `grossReturn` (true pre-cost), `benchmarkReturn`, `netExcessReturn` (null when benchmark unavailable)
...
- **Standardized Walk-Forward**: Ridge with unregularized intercept, train-only standardization, Kendall tau-b Rank IC (6 windows, 2023-10-30 to 2026-06-15)
...
### P1.1 Known: Walk-forward benchmark data needs rebuild
- Old `true_walk_forward_summary.json` has null `benchmarkTradeCount/benchmarkUnavailableCount` — `benchmarkStatus=unavailable` (correct for old data)
- After rebuild with new simulator, benchmarkStatus will switch to `available` (tradeCount>0)
...
|---------|------|-------------|
| v3.4.9.7-COND-T1fix | 2026-06-24 | **P0.2 T1 residual null semantics**: normalizeBenchmarkFields() — all benchmark return/excess null when unavailable, avgNetExcess only from available windows, Cockpit Excess=N.A. gray, 17/17 regression tests pass |
| v3.4.9.7-COND | 2026-06-24 | **P0.2 CONDITIONAL**: benchmark availability gate, coverage dual-constraint rename (649=636+13, 636=636+0), cohort API一致性 (legacyNoTargetDate filter), release identity (43c4f8e matched) |
