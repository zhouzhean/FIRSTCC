...
- **Release identity**: Cloud `buildCommit` = `1039c4a` (P1.7 intraday fix), NOT old `9c3be3c` (P1.6). Manifest has 12 files, includes `simfolio.js` + `scheduler.js`.
- **Intraday fix** (2026-06-26 10:25 CST): Supervisor flagged that 10:00 mid_scan still wrote `expectedReturn=null`. Investigation: `systemctl restart mosaic` at 10:05:39 was AFTER 10:00 mid_scan (10:00:48). Both 09:30 full + 10:00 mid ran on P1.6 code. Two additional fixes deployed:
  1. `scheduler.js`: mid_scan `scheduledSlot` was hard-coded `null` → now passes actual `HH:MM` (e.g. "10:30").
  2. `simfolio.js`: P1.7 block's `_appendPredictionLedger` context now explicitly passes `buildCommit`.
- **Verification**: Cloud test with real 2026-06-26 ledger data (23 entries) → all 23 would get `predictionValid=true`, `expectedReturn` (e.g. 002568 er=0.35, 601991 er=0.31), `confidence=0.5`, 6 breakdown dimensions. `rankByExpectedReturn` confirmed working on mid_scan-format data (cloud live test).
- **Acceptance pending**: 10:30 (or subsequent) intraday mid_scan must produce `expectedReturnInjected>0`, `predictionSource="rankByExpectedReturn"` for intraday path. Then next trading day 09:30 must produce `predictionValid>0`, `researchEligible>0` for canonical path. H2 gated until both.
- **Commits**: `840800f` (code + diagnostic) + `e5a9699` (fix double-count) + `ed912cb` (manifest) + `1039c4a` (intraday: scheduledSlot + buildCommit) + `7284863` (manifest update).
