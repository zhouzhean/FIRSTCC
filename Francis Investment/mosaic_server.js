...
const server = http.createServer(async function(req, res) {
  ...
  // P0-C.1: Delegates to shared cohort_stats.js — single source of truth
  if (pathname === '/api/prediction-settlement') {
    try {
      var csToday = new Date().toISOString().slice(0, 10);
      var cohortStats = require('./mosaic/research/cohort_stats');
      var psRes = cohortStats.buildPredictionSettlementStats(DATA_DIR, csToday);
  ...
  if (pathname === '/api/cohort-integrity') {
    try {
      ...
      var cohortStats = require('./mosaic/research/cohort_stats');
      var ciRes = cohortStats.buildCohortIntegrityStats(DATA_DIR, ciToday);
